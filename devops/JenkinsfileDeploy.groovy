...
def projectDirectory = 'test-hiveconfig5'
def jenkinsSteps
// Set URL of ArgoCD server
def argocdUrl = ''
def argocdAppName = 'test-hiveconfig5'
def argocdNamespace = 'argocd'
def argocdDestinationServer = 'https://kubernetes.default.svc'
def argocdBranch = '${branch}'
def argocdSyncLabel = 'argocd.argoproj.io/instance=test-hiveconfig5'
            ...
            try {
                dir(projectDirectory) {
                    slackSend color: "warning",
                            message: "Test hive config5 \
                       -DartifactId=test-hiveconfig5’ authenticating"
                    jenkinsSteps.argocdAuthenticate(argocdUrl, '${argocdPassword}')
                }
            } catch (err) {
                slackSend color: "danger",
                        message: "Test hive config5 \
                       -DartifactId=test-hiveconfig5’ failed to authenticate properly"
                throw err
            }
        }
    }
    ...
    node('master') {
        try {
            dir(projectDirectory) {
                slackSend color: "warning",
                        message: "Test hive config5 \
                       -DartifactId=test-hiveconfig5’ deploying"
                ...
                slackSend color: "good",
                        message: "Test hive config5 \
                       -DartifactId=test-hiveconfig5’ deployed successfully"
            }
        } catch (err) {
            slackSend color: "danger",
                    message: "Test hive config5 \
                       -DartifactId=test-hiveconfig5’ failed to deploy properly"
            throw err
        }
    }
}
    ...
    node('master') {
        try {
            dir(projectDirectory) {
                slackSend color: "warning",
                        message: "Test hive config5 \
                       -DartifactId=test-hiveconfig5’ shutting down"
                jenkinsSteps.argocdTerminate(argocdAppName)
            }
        } catch (err) {
            slackSend color: "danger",
                    message: "Test hive config5 \
                       -DartifactId=test-hiveconfig5’ failed to shut down properly"
            throw err
        }
    }
}
